...
#define SYSPCLK        (OSC_CLK*(MAIN_PLL_M_Val+1))
#endif
/*----------------------------------------------------------------------------
...
void Main_PLL_Setup ( void )
{
  ...
  regVal &= ~0x1FF;
  LPC_SYSCON->SYSPLLCTRL = (regVal | (MAIN_PLL_P_Val<<5) | MAIN_PLL_M_Val);//M=4,p=2
...
extern uint32_t getSysTICs(void)
{
    uint32_t now;
    now = SysTick->VAL;
    if(now > lastT0TC)
    ...
    {
    sysTICs += (uint32_t)(lastT0TC-now );
    }
    lastT0TC = now;
    return sysTICs;
}

extern uint32_t getElapsedSysTICs(uint32_t startTime)
{
