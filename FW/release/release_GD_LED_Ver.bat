...
copy ..\src\Project\EWARMv5\T6_GD_LED_release.ewp ..\src\Project\EWARMv5\T6.ewp
...
rem 将需要发布的文件进行压缩
echo 正在打包发布文件...
set packetname=HJ58B(LED版本)_生产V"%version_num%"-"%date:~0,4%%date:~5,2%%date:~8,2%".rar
rar a -as -ep1 "%packetname%" "%cd%\update.bin" "%cd%\resdata.bin" "%cd%\Version.txt" "%cd%\升级说明.txt"

copy %packetname% version\%packetname%

set packetname=HJ58B(LED版本)_用户V"%version_num%"-"%date:~0,4%%date:~5,2%%date:~8,2%".rar
